...
2.  **应用规则进行拆解与组合**：
    ...
    *   它会根据规则的类型 (`MIN` 或 `MAX`) 计算出最多可以形成多少个“组合产品”。
    *   **`MIN` 类型规则** (例如：标准左右门配对):
        *   **业务示例** (原始订单: 30个左门, 20个右门):
            *   规则: “左门成品 (FG_DOOR_L)” + “右门成品 (FG_DOOR_R)” = “左右门组合成品 (FG_DOOR_PAIR)”, 类型: `MIN`。
    ...
    *   **`MAX` 类型规则** (例如：优先满足某一侧的大量订单):
        *   **业务场景**: 假设有另一种特殊门 `FG_DOOR_L_MAX` 和 `FG_DOOR_R_MAX`，它们可以组合成 `FG_DOOR_PAIR_MAX`。此规则的目的是，如果客户大量订购了某一侧的门（例如左门远多于右门），但供应商的生产或物料供应倾向于成对生产时，可以按较大数量形成组合，不足的另一侧视为按需补齐或有特殊处理（例如，`FG_DOOR_PAIR_MAX` 的BOM可能不同，或者其成本结构能吸收这种不平衡）。
        *   **业务示例** (原始订单: 30个 `FG_DOOR_L_MAX`, 5个 `FG_DOOR_R_MAX`):
            *   规则: “`FG_DOOR_L_MAX`” + “`FG_DOOR_R_MAX`” = “`FG_DOOR_PAIR_MAX`”, 类型: `MAX`。
            *   系统计算出最多可以组成 `max(30, 5) = 30` 个“`FG_DOOR_PAIR_MAX`”。
            *   形成30个“`FG_DOOR_PAIR_MAX`”后：
                *   消耗了 `min(30, 30) = 30` 个 `FG_DOOR_L_MAX`。
                *   消耗了 `min(5, 30) = 5` 个 `FG_DOOR_R_MAX`。
            *   剩余：`30 - 30 = 0` 个 `FG_DOOR_L_MAX`，`5 - 5 = 0` 个 `FG_DOOR_R_MAX`。
        *   优化后订单片段: 30个 `FG_DOOR_PAIR_MAX`。
        *   **重要说明**: `MAX` 规则的物料影响需要在 `FG_DOOR_PAIR_MAX` 的BOM中体现。例如，如果形成30个 `FG_DOOR_PAIR_MAX` 确实意味着需要30套左门组件和30套右门组件的原材料（即使原始订单只有5个右门），那么其BOM应包含这些。或者，如果 `MAX` 规则下的组合产品有特殊的物料节省（比如只需要30个左门组件和5个右门组件的特定部分，再加上一些通用件），BOM也应反映这一点。这里的示例代码在 `InMemoryDataStore` 中为 `FG_DOOR_PAIR_MAX` 设置了一个BOM，您可以根据实际业务调整。
...
3.  **生成优化后的订单**：
    ...
    *   **代码映射**：`SubcontractingService.optimizeOrderItems()` 方法负责这个逻辑。它会：
        ...
        *   遍历规则，根据规则的 `type` (MIN/MAX) 对订单中的物料进行配对和数量调整。
        *   输出一个优化后的订单项列表。
